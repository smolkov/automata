...
pub async fn signal(path: &str) -> Failliable<Signal> {
    let path = PathBuf::from(path).join("SIGNAL");
    let signal  = Signale::load_no_fallback(path)?;
    Ok(sensor)
}

pub async fn config(path: &str) -> Failliable<Sensor> {
    let path = PathBuf::from(path).join("VALUE");
    let sensor = Sensor::load_no_fallback(path)?;
    Ok(sensor)
}

pub async fn is_ndir(path: &str) -> bool {
    let path = PathBuf::from(path).join("NDIR");
...
pub async read_config(node: Node<Sensor>) {
