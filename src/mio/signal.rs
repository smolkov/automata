...
impl Sensor {
    ...
    pub fn signal_socket(&self) -> PathBuf {
        self.path.join("signal")
    }
}
...
pub async fn signal_brocker(mut stream :UnixStream) -> Result<()> {
    loop {
        let sig: = serialize(&Signal::default())?;
...
pub async fn signal_listner(sensor: &Sensor ) -> Result<()> {
    let listener = UnixListener::bind(&sensor.signal_socket()).await?;
    let mut incoming = listener.incoming();
    while let Some(stream) = incoming.next().await {
        let mut stream = stream?;
        task::spawn(signal_brocker(stream));
    }
    Ok(())

}

pub async fn signal_stream(sensor: &Sensor) -> io::Result<UnixStream> {
    UnixStream::connect(&sensor.signal_socket()).await
}


// pub async fn signal_serve(sensor: &Sensor ) -> Result<()> {
    // let listener = UnixListener::bind("/tmp/sensor").await?;
    // let mut incoming = listener.incoming();
    // while let Some(stream) = incoming.next().await {
        // let mut stream = stream?;
        // task::spawn(signal_writer(stream));
    // }
    // Ok(())
// }
