...
/// Device get status
pub async fn get_device(_cx: Context<Repo>) -> EndpointResult {
    Ok(response::json(analyzer::device::get_local().await.unwrap()))
}
/// Device get serial number
pub async fn get_device_serial(_cx: Context<Repo>) -> EndpointResult {
    Ok(response::json(analyzer::device::get_local().await.unwrap().get_serial()))
}
/// Divice set serial number
pub async fn set_device_serial(mut cx:Context<Repo>) -> EndpointResult<()> {
...
pub async fn get_streams_list(_cx: Context<Repo>) -> EndpointResult {
    // let app = cx.state();
...
pub async fn get_stream(cx: Context<Repo>) -> EndpointResult {
    let number = cx.param("number").client_err()?;
    let stream = analyzer::stream::get_stream(number).await.unwrap();
    Ok(response::json (stream))
}
pub async fn set_stream(mut cx: Context<Repo>) -> EndpointResult<()> {
    let stream = cx.body_json().await.client_err()?;
    analyzer::stream::set_stream(stream).await.unwrap();
    Ok(())
}
// pub async fn set_stream_name(_cx: Context<Repo>) -> EndpointResult<()> {
    // Ok(())
// }

pub async fn get_rules(_cx: Context<Repo>) -> EndpointResult {
    let rules = analyzer::rules::find_all().await.unwrap();
    Ok(response::json(rules))
}

pub async fn set_rule(mut cx: Context<Repo>) -> EndpointResult<()> {
