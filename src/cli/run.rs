use super::exercise::*;
use super::verify::test;
use console::{style, Emoji};
use indicatif::ProgressBar;

pub async fn run(exercise: &Exercise) -> Result<(), ()> {
    match exercise.mode {
        Mode::Test => test(exercise)?,
        Mode::Compile => compile_and_run(exercise).await?,
    }
    Ok(())
}

pub async fn compile_and_run(exercise: &Exercise) -> Result<(), ()> {
